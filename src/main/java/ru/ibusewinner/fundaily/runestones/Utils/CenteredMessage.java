...
public class CenteredMessage {
    ...
    public static String getCenteredMessage(String translateAlternateColorCodes) {
        if (translateAlternateColorCodes == null || translateAlternateColorCodes.equals("")) {}
        assert translateAlternateColorCodes != null;
        translateAlternateColorCodes = ChatColor.translateAlternateColorCodes('&', translateAlternateColorCodes);
        int n = 0;
        int n2 = 0;
        boolean b = false;
        char[] charArray;
        for (int length = (charArray = translateAlternateColorCodes.toCharArray()).length, i = 0; i < length; ++i) {
            ...
            if (c == '§') {
                ...
                final DefaultFontInfo defaultFontInfo = DefaultFontInfo.getDefaultFontInfo(c);
                n += (b ? defaultFontInfo.getBoldLength() : defaultFontInfo.getLength());
                ++n;
            }
        }
        final int n3 = 154 - n / 2;
        final int n4 = DefaultFontInfo.SPACE.getLength() + 1;
        int j = 0;
    ...
    public enum DefaultFontInfo
    {
        ...
        public static DefaultFontInfo getDefaultFontInfo(final char c) {
            ...
            for (int length = (values = values()).length, i = 0; i < length; ++i) {
                final DefaultFontInfo defaultFontInfo = values[i];
